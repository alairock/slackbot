...
from smackbot.slackclient import SmackClient
from . import slackclient_data


@pytest.fixture
def slack_client():
    c = SmackClient(None, connect=False)
...
def test_find_channel_by_name(slack_client):
    assert slack_client.find_channel_by_name('smackbot') == 'D0X6385P1'
...
def test_find_user_by_name(slack_client):
    ...
    assert slack_client.find_user_by_name('smackbot') == 'USMACKBOT'
