...
def setup_aliases(monkeypatch):
    monkeypatch.setattr('smackbot.settings.ALIASES', ','.join(TEST_ALIASES))


@pytest.fixture()
def dispatcher(monkeypatch):
    monkeypatch.setattr('smackbot.settings.DEFAULT_REPLY', 'sorry')
    dispatcher = smackbot.dispatcher.MessageDispatcher(None, None, None)
...
def test_dispatch_msg(dispatcher, monkeypatch):
    monkeypatch.setattr('smackbot.dispatcher.Message', FakeMessage)
